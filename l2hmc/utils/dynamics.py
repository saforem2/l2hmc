...
class Dynamics(object):
    def __init__(self,
                 ...
                 #  update_trajectory_length=False):
    ...
    def _init_mask(self):
        ...
        for t in range(self.trajectory_length):
            ind = np.random.permutation(np.arange(self.x_dim))[:int(self.x_dim
                                                                    / 2)]
...
#       def forward(self, x, init_v=None):
    ...
    def backward(self, x, init_v=None, aux=None, log_jac=False):
        ...
        def body(x, v, t, j):
            new_x, new_v, log_j = self._backward_step(x, v, self.trajectory_length -
                                                      t - 1, aux=aux)
            return new_x, new_v, t+1, j+log_j
