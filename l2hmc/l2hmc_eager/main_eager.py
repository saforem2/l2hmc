...
import .dynamics_eager as l2hmc
...
def main(_):
    ...
    energy_fn, mean, covar = {
        ...
        #  "gmm": dynamics_eager.get_gmm_energy_fn()
    }[FLAGS.energy_fn]
    ...
    if FLAGS.train_dir:
        ...
        if FLAGS.restore:
            ...
            print(f"Restored latest checkpoint at path: {latest_path}")
            sys.stdout.flush()

    if not FLAGS.restore:
        # Training
        if FLAGS.use_defun:
            # Use `tfe.defun` to boost performance when there are lots of small
            # ops
        ...
        for i in range(1,  train_iters + 1):
            ...
            if i % record_loss_every == 0:
                print("Iteration {}, loss {:.4f}, x_accept {:.4f}".format(
                    i, loss.numpy(),
        ...
        if FLAGS.train_dir:
            ...
            print(f"Saved checkpoint as path: {saved_path}")
            sys.stdout.flush()

    # Evaluation
    if FLAGS.use_defun:
        # Use tfe.defun to boost performance when there are lots of small ops
    ...
    ess = compute_ess(acl_spectrum)
    print("Effective sample size per Metropolis-Hastings step: {}".format(ess))
    sys.stdout.flush()

    if FLAGS.train_dir:
        # Plot autocorrelation spectrum in tensorboard
        plot_step = tfe.Variable(1, trainable=False, dtype=tf.int65)

        for ac in ac_spectrum:
            with summary_writer.as_default():
                with tf.contrib.summary.always_record_summaries():
                    tf.contrib.summary.scalar("Autocorrelation", ac,
                                              step=plot_step)
            plot_step.assign(plot_step + n_steps)

        if HAS_MATPLOTLIB:
            # Choose a single chain and plot the trajectory
            single_chain = samples_history[:, 0, :]  # trained chain
            xs = single_chain[:100, 0]
            ys = single_chain[:100, 1]
            plt.figure()
            plt.plot(xs, ys, color='C1', marker='o', alpha=0.6)
            plt.savefig(os.path.join(FLAGS.train_dir, "single_chain.pdf"),
...
def compute_ac_spectrum(samples_history, target_mean, target_covar):
    ...
    Follows Eq. 15 from the L2HMC paper.

    Args:
        samples_history (np.ndarray, shape=[T, B, D]):
            Where T is the total number of time steps, B is the batch size, and
            D is the dimensionality of the sample space.
        target_mean (np.ndarray):
            1D Numpy array of the mean of target (true) distribution.
        target_covar (np.ndarray):
            2D Numpy array representing a symmetric matrix for variance.
    Returns:
    ...
    for t in range(time_steps - 1):
        ...
        for tau in range(time_steps - t):
            ...
            # Take dot product over observation dims and 
            # take mean over batch dims
...
def compute_ess(ac_spectrum):
    ...
    This follows Eq. 16 from the L2HMC paper.
    ...
    cutoff = np.argmax(ac_spectrum[1:] < 0.05)
    if cutoff == 0:
...
if __name__ == "__main__":
    ...
    flags.DEFINE_boolean(
        "restore",
        default=False,
        help="[Optional] Restore the latest checkpoint from"
        "`train_dir` if True"
    )
    flags.DEFINE_boolean(
        "use_defun",
